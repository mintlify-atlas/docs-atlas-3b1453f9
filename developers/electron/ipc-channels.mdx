...
### `wzrd:media:resolve-url`

Validates and resolves a `wzrd://media/?file=…` URL for playback. Returns the URL string if the path is in the allow-list.
...
### `wzrd:clip-studio:youtube-info`

Fetches YouTube video metadata using `yt-dlp --dump-single-json` without downloading the video.

```typescript
// IPC channel: wzrd:clip-studio:validate-youtube-downloader  (health check)
...
### `wzrd:clip-studio:youtube-subtitles`

Downloads English auto-generated or manual subtitles (VTT format) for a YouTube video using `yt-dlp --write-auto-subs --write-subs --skip-download`. Called internally by `wzrd:clip-studio:youtube-download` — you rarely need to call this directly.
