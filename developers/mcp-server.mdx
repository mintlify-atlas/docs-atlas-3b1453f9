...
<Tip>
  Before calling any generation tool, call `get_credits` first to verify the user has sufficient balance. Generation tools return a `402` with `{ code, required, available, top_up_url }` when credits run out — surface `top_up_url` to the user rather than failing silently.
</Tip>
