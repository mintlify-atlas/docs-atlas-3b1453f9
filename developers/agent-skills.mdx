...
<Accordion title="make-magic — End-to-End Project to Director's Cut">
  The highest-level skill. Accepts a raw concept and format string, creates a project, generates a full storyline, renders all shots, and produces a final Director's Cut video — all in a single automated pipeline. Each step deducts credits; check `/billing/credits` before starting.
