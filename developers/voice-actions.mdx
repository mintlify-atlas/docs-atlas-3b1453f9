...
export function VoiceAgentProvider({ children }: { children: React.ReactNode }) {
  ...
  // Register global navigation actions on every route change
  ...
  useRegisterVoiceActions([
    {
      ...
      scope: 'storyboard-page',
      description: 'Generate all storyboard images for the current project',
      confirmation: {
        risk: 'generation',
        message: 'This will use credits to generate all storyboard images. Continue?',
      },
...
<Note>
  For local development you can set `VITE_OPENAI_API_KEY` in your `.env.local` file to bypass the edge function. In production, keys must route through the `realtime-client-secret` Supabase Function — never expose service tokens in the renderer.
</Note>
