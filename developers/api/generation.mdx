...
<ResponseField name="visual_prompt" type="string | null">
  Full enriched visual prompt for image generation (available from `enriching` phase).
</ResponseField>
...
<Steps>
  <Step title="creating">
    Shot row inserted. `title` and `visual_prompt` are `null`.
  </Step>
  <Step title="drafting">
    `prompt_idea` and `shot_type` written to the database. `title` populated.
  </Step>
  <Step title="enriching">
    `visual_prompt` written; `image_status` set to `prompt_ready`.
  </Step>
  <Step title="ready">
...
Monitor progress in real time by subscribing to the SSE stream:

```
GET /functions/v1/project-auto-generate/stream?project_id=<project-id>
Authorization: Bearer <supabase-jwt>
Accept: text/event-stream
```
