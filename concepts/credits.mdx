---
title: "Credits: WZRD Studio AI Usage Metering"
...
- **API** — poll `GET /functions/v1/billing-portal` to retrieve `{ subscription, credits }` programmatically.
...
<Steps>
  ...
  <Step title="Choose a plan or credit pack">
    Select a subscription plan (`plan`) or a one-time credit pack (`pack`). Available codes include `pro` for the Pro plan and `pack_100` for a 100-credit top-up.
  </Step>
  ...
  <Step title="Credits are applied instantly">
    Once payment completes, your balance updates immediately. Poll `GET /functions/v1/billing-portal` to confirm the new credit total.
  </Step>
</Steps>
...
```bash
# Poll plan and credit status
curl https://<project>.supabase.co/functions/v1/billing-portal \
  -H "Authorization: Bearer <supabase-jwt>"

# Response
{ "subscription": { ... }, "credits": 97 }
```

<Tip>
  The `billing-checkout` agent skill wraps this endpoint for use in automated workflows. Pass `{ "kind": "plan" | "pack", "code": "pro" | "pack_100" }` to the skill and open the returned `checkout_url` in a browser window.
</Tip>
