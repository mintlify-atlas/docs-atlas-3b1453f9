...
IP Vault wraps the `@story-protocol/core-sdk` behind a clean service layer. The `ipVaultService` handles all Supabase interactions, while the `registerVaultItemOnStory` helper (in `src/lib/story/registration`) calls the Story Protocol SDK with a connected wallet. Metadata is pinned to IPFS via the `story-ipfs-metadata` Supabase edge function before any on-chain transaction is submitted.
...
<Steps>
  <Step title="Add an asset to IP Vault">
    ...
    The service resolves the source record, copies its media URL, thumbnail, and metadata into a new `ip_vault_items` row with `registration_status: "draft"`, and builds an initial **proof packet** capturing provenance.
  </Step>
...
<CardGroup cols={2}>
  ...
  <Card title="Commercial Remix" icon="code-branch">
    Allows commercial derivative works. Configurable `commercial_rev_share` (%) and `minting_fee_wip`.
  </Card>
