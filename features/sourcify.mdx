...
<CardGroup cols={2}>
  ...
  <Card title="YouTube Shorts" icon="youtube">
    Same underlying actor, tuned with `duration: "s"` and `includeShorts: true` for short-form YouTube discovery.
  </Card>
  ...
  <Card title="TikTok Fast" icon="tiktok">
    Hashtags, creator profiles, keyword searches, and direct video URLs. Actor ID configurable via `SOURCIFY_TIKTOK_ACTOR_ID`.
  </Card>
...
<Steps>
  ...
  <Step title="Review the Codex plan">
    The `sourcify-apify` function calls the **Codex planner** — an OpenAI Responses API call with a structured JSON schema — to decompose your prompt into 1–5 focused scrape targets. Each target is matched to the most appropriate Apify actors.

    If the Codex planner is unavailable (no API key, or rate-limited), the function falls back to a **deterministic plan** that constructs sensible actor inputs directly from the topic string.
  ...
  <Step title="Fetch MP4 downloads (optional)">
    ...
    <Warning>
      Fetching MP4s triggers paid Apify actor runs. A maximum of **12 items** are processed per batch to control costs. You can adjust the **Max charge (USD)** setting to cap total Apify spend per scrape session.
    </Warning>
  </Step>
...
<Tabs>
  <Tab title="plan">
    Generates a scraping plan without running any actors. Uses the Codex planner when `OPENAI_API_KEY` and `WZRD_AGENT_PROVIDER=codex` are configured; otherwise falls back to a deterministic plan.
  ...
  <Tab title="finalize">
    Downloads media files from selected results, uploads them to the `project-assets` Supabase Storage bucket, and creates `project_assets` table rows with full Sourcify provenance metadata.
...
<Note>
  Sourcify requires an **Apify API token** configured as the `APIFY_API_TOKEN` environment variable on the edge function. Without it, `plan` will succeed (Codex planning is independent), but `run` and `results` actions will return an error.
</Note>
